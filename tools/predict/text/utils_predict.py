...
def check_args(args):
    if not args.raw_data_dir:
        print("WARNING: The 'raw_data_dir' is empty. The detection predictor will load the data from 'dataset_root/data_dir' in det yaml file. ")
    else:
    ...
    if args.crop_save_dir:
        ...
        print("WARNING: The 'crop_save_dir' is empty. The recognition predictor will load the data from 'dataset_root/data_dir' in rec yaml file.")
    ...
    with open(save_path, 'w') as f:
        for ori_img_name, crop_content in box_dict.items():
            ...
            for crop_img_name, box in crop_content.items():
                line.append({"transcription": rec_text_dict[crop_img_name],
                             "points": box.reshape(-1,).tolist()})
...
def rescale(det_pred_outputs):
    # assert len(det_pred_outputs['pred_images']) == len(det_pred_outputs['raw_imgs_shape'])  # TODO: can do in BasePredict
    assert len(det_pred_outputs['pred_images']) == len(det_pred_outputs['predicted_boxes']), \
           "The number of images before and after detection prediction doesn't match. Please check the detection prediction results."

    imgnames = [os.path.basename(img_path.asnumpy()[0]) for img_path in det_pred_outputs['img_paths']]  # TODO: can do in BasePredict
